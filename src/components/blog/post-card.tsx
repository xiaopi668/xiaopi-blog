...
export function PostCard({ post }: PostCardProps) {
  return (
    <article className="group relative rounded-2xl glass overflow-hidden border hover:border-primary/30" style={{ transition: "all .5s" }} onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-4px)"; e.currentTarget.style.boxShadow = "0 8px 40px rgba(99,102,241,0.15)"; }} onMouseLeave={e => { e.currentTarget.style.transform = ""; e.currentTarget.style.boxShadow = ""; }}>
      <Link href={`/posts/${post.slug}`} className="block">
        ...
        <div className="p-5">
          ...
          <div className="mt-3 flex items-center gap-1 text-xs text-primary opacity-0 translate-x-[-4px] transition-all duration-300 group-hover:opacity-100 group-hover:translate-x-0">
            阅读更多
